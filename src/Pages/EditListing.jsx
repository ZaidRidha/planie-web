  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Existing uploads come back as Storage URLs; keep them (no File —
      ...
      <div className="pd-layout">
        ...
        <main className="pd-main">
          <div className="al-success pd-animate pd-d1">
            {loading ? (
              <p style={{ color: "#687076" }}>Loading listing…</p>
            ) : (
              <>
      ...
      <main className="pd-main">
        ...
        <form onSubmit={handleSubmit} className="al-form">
          ...
          <section className="pd-card al-section pd-animate pd-d2">
            ...
            <div className="al-field">
              <label className="al-label">Primary category * <span style={{ fontWeight: 400, opacity: 0.55 }}>(pick one)</span></label>
              <span className="al-help">
                Choose the single category that best describes your venue's primary identity — this determines where you appear across browse and search. You only pick one on purpose: the occasions you selected above already capture the other vibes your venue fits, so accuracy here beats breadth.
              </span>
              <div className="al-category-grid">
                {categories.map((cat) => (
                  <button
              ...
              {imagePreview.map((img, i) => (
                <div key={i} className="al-photo-thumb">
                  <img
                    src={img.url}
                    alt={img.name}
                    onError={(e) => { e.currentTarget.style.opacity = 0; e.currentTarget.parentElement.style.background = "rgba(28,17,20,0.06)"; }}
                  />
          ...
          <div className="al-submit-row pd-animate pd-d4">
            ...
            <button type="submit" className={`pd-btn pd-btn--primary al-submit-btn${saving ? " is-busy" : ""}`} disabled={saving}>
              {saving ? <>Saving<span className="busy-dots" /></> : "Save Changes"}
            </button>
          </div>
          {saveError && (
